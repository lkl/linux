...
struct lkl_cpu {
	...
	/* semaphore to wait for waiting threads to shutdown */
	struct lkl_sem *drain_sem;
	/* semaphore used for shutdown */
	struct lkl_sem *shutdown_sem;
} cpu;

static int __cpu_try_get_lock(void)
{
	lkl_thread_t self;

	if (__sync_fetch_and_add(&cpu.shutdown_gate, 1) >= MAX_THREADS)
		return -1;
...
static void __cpu_try_get_unlock(void)
{
	lkl_ops->mutex_unlock(cpu.lock);
	__sync_fetch_and_sub(&cpu.shutdown_gate, 1);
}
...
int lkl_cpu_init(void)
{
	...
	if (!cpu.lock || !cpu.sem || !cpu.idle_sem || !cpu.drain_sem ||
	    !cpu.shutdown_sem) {
