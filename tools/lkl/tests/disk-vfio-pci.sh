...
{
    # initialize
    dd if=/dev/zero of=$script_dir/nvme.img bs=1024 count=102400
    yes | sudo mkfs.$fstype $script_dir/nvme.img
    ...
    scp -i $script_dir/id_rsa -P 2222 $script_dir/$bin_name lkl@localhost:
}
...
function run()
{
	...
	lkl_test_run 1 init
	lkl_test_exec $SSH_QEMU ./$bin_name -n 0000:00:03.0 -t $fstype
	lkl_test_plan 1 "disk-vfio-pci $fstype"
