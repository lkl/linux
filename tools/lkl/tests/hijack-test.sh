...
if [[ ! -e ${basedir}/lib/hijack/liblkl-hijack.so ]]; then
    lkl_test_plan 0  "missing liblkl-hijack.so"
    exit 0
fi
...
if [ -z $(lkl_test_cmd which mkfifo &>/dev/null) ]; then
    lkl_test_plan 0 "hijack pipe backend tests: no mkfifo command"
else
...
if ! lkl_test_cmd test -c /dev/net/tun &>/dev/null; then
    lkl_test_plan 0 "hijack tap backend tests: missing /dev/net/tun"
else
...
if [ -z "$LKL_HOST_CONFIG_VIRTIO_NET_VDE" ]; then
    lkl_test_plan 0 "vde not supported"
elif [ ! -x "$(which vde_switch)" ]; then
    lkl_test_plan 0 "could not find a vde_switch executable"
else
    lkl_test_plan 3 "hijack vde backend tests"
