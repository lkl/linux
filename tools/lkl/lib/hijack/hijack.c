...
int socket(int domain, int type, int protocol)
{
	...
	ret = lkl_sys_wrapper_socket(domain, type, protocol);
	if (ret >= 0)
...
int close(int fd)
{
	...
	return lkl_sys_wrapper_close(fd - LKL_FD_OFFSET);
}

ssize_t recvmsg(int fd, struct msghdr *msghdr, int flags)
{
	return lkl_sys_wrapper_recvmsg(fd - LKL_FD_OFFSET, msghdr, flags);
}
...
ssize_t sendmsg(int fd, const struct msghdr *msghdr, int flags)
{
	...
	return lkl_sys_wrapper_sendmsg(fd - LKL_FD_OFFSET,
...
int sendmmsg(int fd, struct mmsghdr *msghdr, unsigned int vlen,
		  int flags)
{
	return lkl_sys_wrapper_sendmmsg(fd - LKL_FD_OFFSET, msghdr,
					vlen, flags);
}

int getsockname(int fd, struct sockaddr *name, socklen_t *namelen)
{
	return lkl_sys_wrapper_getsockname(fd - LKL_FD_OFFSET, name,
					   (int *)namelen);
}

int getpeername(int fd, struct sockaddr *name, socklen_t *namelen)
{
	return lkl_sys_wrapper_getpeername(fd - LKL_FD_OFFSET, name,
					   (int *)namelen);
}
...
int bind(int fd, const struct sockaddr *name, socklen_t namelen)
{
	...
	return lkl_sys_wrapper_bind(fd - LKL_FD_OFFSET, (struct sockaddr *)name,
				    namelen);
}

int connect(int fd, const struct sockaddr *addr, socklen_t len)
{
	return lkl_sys_wrapper_connect(fd - LKL_FD_OFFSET,
...
int listen(int fd, int backlog)
{
	...
	return lkl_sys_wrapper_listen(fd - LKL_FD_OFFSET, backlog);
}
...
int shutdown(int fd, int how)
{
	...
	return lkl_sys_wrapper_shutdown(fd - LKL_FD_OFFSET, how);
}
...
int accept(int fd, struct sockaddr *addr, socklen_t *addrlen)
{
	...
	return lkl_sys_wrapper_accept(fd - LKL_FD_OFFSET, addr, (int *)addrlen);
}
...
ssize_t write(int fd, const void *buf, size_t count)
{
	...
	return lkl_sys_wrapper_write(fd - LKL_FD_OFFSET, (void *)buf, count);
}
...
ssize_t writev(int fd, const struct iovec *iov, int count)
{
	...
	return lkl_sys_wrapper_writev(fd - LKL_FD_OFFSET, (void *)iov, count);
}
			...
			const struct sockaddr *dest_addr, unsigned int addrlen)
{
	...
	return lkl_sys_wrapper_sendto(fd - LKL_FD_OFFSET, (void *)buf, len,
...
ssize_t read(int fd, void *buf, size_t count)
{
	...
	return lkl_sys_wrapper_read(fd - LKL_FD_OFFSET, (void *)buf, count);
}

ssize_t recvfrom(int fd, void *buf, size_t len, int flags,
		      struct sockaddr *from, socklen_t *fromlen)
{
	return lkl_sys_wrapper_recvfrom(fd - LKL_FD_OFFSET, buf, len, flags,
					from, (int *)fromlen);
}
	       ...
	       const void *optval, socklen_t optlen)
{
	...
	return lkl_sys_wrapper_setsockopt(fd - LKL_FD_OFFSET, level, optname,
					  (void *)optval, optlen);
}
...
{
	return lkl_sys_wrapper_getsockopt(fd - LKL_FD_OFFSET, level, optname,
					  optval, (int *)optlen);
}
...
int ioctl(int fd, unsigned long int request, ...)
{
	...
	return lkl_sys_wrapper_ioctl(fd - LKL_FD_OFFSET, request,
				     (unsigned long)argp);
}
...
int fcntl(int fd, int cmd, ... /* arg */)
{
	...
	return lkl_sys_wrapper_fcntl(fd - LKL_FD_OFFSET, cmd,
				     (unsigned long)argp);
}
...
{
	return lkl_sys_wrapper_pipe(pipefd);
}
...
poll(struct pollfd *fds, nfds_t nfds, int timeout)
{
	...
	return lkl_sys_wrapper_poll(lkl_fds, nfds, timeout);
}
	...
	fd_set *exceptfds, struct timeval *timeout)
{
	...
	return lkl_sys_wrapper_select(nfds, readfds, writefds,
...
epoll_create(int size)
{
	...
	ret = lkl_sys_wrapper_epoll_create(size);
	if (ret >= 0)
		return ret + LKL_FD_OFFSET;
...
{
	return lkl_sys_wrapper_epoll_ctl(epollfd - LKL_FD_OFFSET, op,
...
{
	/* FIXME: need to handle mixed case of hostfd and lklfd. */
	return lkl_sys_wrapper_epoll_wait(epollfd - LKL_FD_OFFSET, events,
					  maxevents, timeout);
}
