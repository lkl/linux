...
/*
 * XXX: someone reported that DPDK's mempool with cache is not thread safe
 * (e.g., http://www.dpdk.io/ml/archives/dev/2014-February/001401.html),
 * potentially rte_pktmbuf_alloc() is not thread safe here.
 * so I tentatively disabled the cache on mempool by assigning
