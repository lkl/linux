...
static inline int virtio_add_used(struct virtio_queue *q, uint16_t used_idx,
...
void virtio_req_complete(struct virtio_req *req, uint32_t len)
{
	...
	for (i = 0; i < req->buf_count; i++) {
		...
		if (!q->max_merge_len)
			...
			used_len = min(len,  req->buf[i].iov_len);

		len -= virtio_add_used(q, used_idx++, avail_idx++, used_len);
		if (!len)
			break;
	}
