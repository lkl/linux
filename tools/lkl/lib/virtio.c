...
struct lkl_vring_desc *vring_desc_at_le_idx(struct virtio_queue *q, __lkl__virtio16 le_idx)
{
			      ...
			      int idx)
{
	...
	if (q_buf_cnt == VIRTIO_REQ_MAX_BUFS &&
		...
		lkl_printf("enqueued too many request bufs\n");
		lkl_host_ops.panic();
	}
