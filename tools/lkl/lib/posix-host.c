...
static void *sem_alloc(int count)
{
	...
	sem->count = count;
	WARN_UNLESS(pthread_cond_init(&sem->cond, NULL));
#endif /* _POSIX_SEMAPHORES */
...
#else
	WARN_UNLESS(pthread_mutex_lock(&sem->lock));
	sem->count++;
	if (sem->count > 0)
		WARN_UNLESS(pthread_cond_signal(&sem->cond));
	WARN_UNLESS(pthread_mutex_unlock(&sem->lock));
#endif /* _POSIX_SEMAPHORES */
...
static void sem_down(void *_sem)
{
	...
	WARN_UNLESS(pthread_mutex_lock(&sem->lock));
	while (sem->count <= 0)
		WARN_UNLESS(pthread_cond_wait(&sem->cond, &sem->lock));
	sem->count--;
	WARN_UNLESS(pthread_mutex_unlock(&sem->lock));
#endif /* _POSIX_SEMAPHORES */
}
...
static struct lkl_mutex_t *mutex_alloc(void)
{
	...
	mutex = &_mutex->mutex;
	WARN_UNLESS(pthread_mutexattr_init(&attr));

	/* PTHREAD_MUTEX_ERRORCHECK is *very* useful for debugging,
	 * but has some overhead, so we provide an option to turn it
	 * off. */
#ifdef DEBUG
	WARN_UNLESS(pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_ERRORCHECK_NP));
#endif /* DEBUG */

	WARN_UNLESS(pthread_mutex_init(mutex, &attr));
